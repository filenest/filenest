...
import {
    FolderProvider,
    useFolderContext,
    type FolderInternals,
} from "../____context/local/FolderContext"
...
const Folder = ({ children, asChild, ...props }: Omit<FolderProps, "folder">) => {
    ...
    return (
        <Comp {...props} onClick={navigateTo}>
            {getChildren()}
        </Comp>
    )
}

export interface FolderActionTriggerProps
...
export interface FolderCreateTriggerProps
    extends React.ComponentPropsWithoutRef<"button"> {
    asChild?: boolean
}
