...
} from "../____context/local/AssetContext"
...
const Asset = ({ asset, asChild, children, ...props }: AssetProps) => {
    ...
    function onClick(e: React.MouseEvent<HTMLDivElement>) {
        if (e.ctrlKey || e.metaKey) {
            ...
            const firstSelectedIndex = data.findIndex(
                (a) => a.assetId === firstSelectedFile.assetId
            )
...
export interface AssetActionTriggerProps
    extends React.ComponentPropsWithoutRef<"button"> {
    action: "remove" | "rename" | "select"
    asChild?: boolean
}

export const AssetActionTrigger = ({
    action,
    asChild,
    ...props
}: AssetActionTriggerProps) => {
    const {
        remove,
        rename,
        select,
